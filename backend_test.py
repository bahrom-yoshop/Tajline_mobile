#!/usr/bin/env python3
"""
🎯 КРИТИЧЕСКОЕ ТЕСТИРОВАНИЕ: Исправление статистики для показа только реально занятых ячеек

ПРОБЛЕМА:
Пользователь сообщил что на складе размещено только 3 груза в 2 ячейках:
- Ячейка Б1-П3-Я2: грузы 25082235/01/02 и 25082235/02/01  
- Ячейка Б1-П3-Я3: груз 25082235/01/01

НО карточка склада показывает 14 занятых ячеек вместо 2.

ИСПРАВЛЕНИЕ:
Изменил API `/api/warehouses/{warehouse_id}/statistics` чтобы он считал только актуально размещенные грузы (is_placed=true в operator_cargo), а не все placement_records в базе.

КРИТИЧЕСКИЕ ОЖИДАНИЯ:
- occupied_cells должно быть 2 (НЕ 14!)
- total_placed_cargo должно быть 3
- В логах должно быть видно только грузы 25082235/01/02, 25082235/02/01, 25082235/01/01
"""
