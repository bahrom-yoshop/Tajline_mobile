#!/usr/bin/env python3
"""
🎯 КРИТИЧЕСКОЕ ТЕСТИРОВАНИЕ: Исправления для отображения цены за кг от курьера в модальном окне просмотра принятого уведомления в TAJLINE.TJ

КОНТЕКСТ ТЕСТИРОВАНИЯ:
Протестировать исправления для отображения цены за кг от курьера в модальном окне просмотра принятого уведомления.
Проверить, что в поле "Цена (₽)" теперь отображается именно та стоимость за кг (price_per_kg), которую курьер заполняет при оформлении груза, а не общая стоимость.

ЗАДАЧИ ТЕСТИРОВАНИЯ:
1. Создать тестовую заявку на забор груза с данными курьера, включая поле price_per_kg
2. Проверить, что поле price_per_kg правильно сохраняется в заявке на забор груза 
3. Протестировать endpoint GET /api/operator/pickup-requests/{request_id} для получения данных
4. Убедиться, что в modal_data.cargo_info есть поле price_per_kg
5. Проверить расчет общей суммы: вес × price_per_kg = total_value

ДЕТАЛИ ДЛЯ ПРОВЕРКИ:
- При создании заявки на забор груза поле price_per_kg должно сохраняться
- В endpoint modal_data должно возвращаться поле cargo_info.price_per_kg
- Frontend должен использовать price_per_kg для отображения в поле "Цена (₽)"
- Общая сумма должна рассчитываться как: вес × цена за кг

ПРИМЕР ДАННЫХ ДЛЯ ТЕСТИРОВАНИЯ:
- Груз: "Холодильник"
- Вес: 30 кг
- Цена за кг: 80 ₽ (это должно отображаться в поле "Цена (₽)")
- Общая стоимость: 30 × 80 = 2400 ₽ (это должно показываться как итого)
"""
