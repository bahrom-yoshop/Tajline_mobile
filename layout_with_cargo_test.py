...
class LayoutWithCargoTester:
    ...
    def generate_final_report(self):
        ...
        if overall_success:
            ...
            self.log("📊 Большинство размещенных единиц имеют полную информацию для отображения")
            ...
            if not self.test_results["cargo_info_field_present"]:
                ...
                success_rate = self.test_results["detailed_results"].get("success_rate", 0)
                self.log(f"🔍 ПРОБЛЕМА: Качество данных недостаточное ({success_rate:.1%} < 75%)")
